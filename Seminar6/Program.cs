...
string[] st = ReadLine().Split(' ',StringSplitOptions.RemoveEmptyEntries); // строчный ввод трех чисел Массив (с условием убрать пробел)
...
bool IsTriangle(int a,int b,int c) // прада или нет боол, вводим три переменные
{
    return (((a+b)>c)&&((a+c)>b)&&((b+c)>a)); // условие существования треугольника, каждая сторона меньше суммы двух других
}
...
{
    int[] result = new int[inArray.Length]; //созданеи нового массива
    for (int i = 0; i < inArray.Length; i++)
    {
